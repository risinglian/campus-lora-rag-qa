...
> **真实性说明**：本报告依据远程项目 `/root/autodl-tmp/llm_workspace` 的真实文件、真实运行日志和真实评估结果填写。已核验 LoRA adapter、数据集、RAG 代码、ChromaDB 检索与 12 题三模式评估结果。当前项目实际 RAG embedding 实现为 `HashEmbeddingFunction`，不是 BGE；报告中不把 Hash 伪写为 BGE。若后续改为 BGE，应重新构建向量库并重新运行同一套评估。

---

## 摘要

本项目面向校园服务问答场景，构建了一个结合 LoRA 微调与 RAG 检索增强的校园智能问答助手。项目以 Qwen2.5-1.5B-Instruct 为基础模型，使用 240 条校园问答数据进行 LoRA 参数高效微调，使模型更熟悉校园服务类问题的表达方式和回答结构；同时使用 ChromaDB 构建校园知识库，在推理阶段检索图书馆、教务、校园卡、宿舍等相关资料并注入 Prompt，提升回答的事实依据和可维护性。为验证系统效果，本项目设计了 Base、LoRA、LoRA+RAG 三种模式，并使用 12 个覆盖域内已见、域内泛化、域外拒答、通用能力、指令遵循和综合问题的测试集进行对比评估。实验结果显示，LoRA+RAG 平均分为 4.58，高质量回答率为 91.7%，域内问题平均分达到 5.00，优于 Base 与单独 LoRA，说明“参数高效微调 + 外部知识检索”的组合更适合校园制度类问答任务。

---

## 一、项目背景与选题动机

校园服务信息通常分散在图书馆、教务系统、校园卡平台、宿舍管理、就业服务等多个入口。学生在真实使用时往往不会按照制度文件中的标题提问，而是会用比较口语化的方式表达需求，例如“饭卡不见了先挂失还是补办”“周末晚上还能不能去图书馆自习”“宿舍灯坏了找谁修”。通用大模型虽然具有较强的语言生成能力，但对具体学校制度和流程并不天然掌握，容易给出泛化建议，甚至把不同学校的规则混在一起。对于校园服务类问题，回答不仅要通顺，还要尽量贴近实际办事流程，并在缺乏依据时避免乱编。

因此，本项目选择“校园智能问答助手”作为垂直领域微调任务。项目目标不是训练一个完全替代官方通知的模型，而是在课程实验条件下验证以下问题：第一，LoRA 微调能否让基础模型形成更稳定的校园问答表达格式；第二，RAG 检索能否为制度类问题提供更明确的依据；第三，LoRA 与 RAG 结合后，是否比单独使用基础模型或单独加载 LoRA adapter 更适合校园服务问答。

本项目最终实现了三种可对比的推理模式。Base 模式直接调用 Qwen2.5-1.5B-Instruct，用于观察基础模型原始能力；LoRA 模式加载校园问答微调后的 adapter，用于观察微调对回答风格和领域表达的影响；LoRA+RAG 模式先从 ChromaDB 检索校园知识，再将参考资料与用户问题共同输入 LoRA 模型，用于观察完整系统的实际效果。
...
数据层包含两类数据：一类是用于微调的 `campus_qa.json`，它提供 instruction、input、output 格式的校园问答样本；另一类是用于 RAG 的 `campus_docs.json`，它保存校园服务知识文档。微调层通过 LLaMA-Factory 对基础模型进行 LoRA 训练，只保存 adapter 权重，不复制完整基础模型。知识检索层使用 ChromaDB 持久化校园知识库。推理层由 `main.py` 统一管理三种模式，使同一个问题可以在 Base、LoRA、LoRA+RAG 三种设置下进行公平对比。评估层通过 12 个固定问题和 1～5 分评分标准统计平均分、高质量回答率、域内得分、域外拒答成功率和指令遵循表现。

这种架构的优点是分工清晰。LoRA 负责让模型学会校园服务问答的表达习惯，例如先说明处理步骤、再提示以官方通知为准；RAG 负责提供可更新的知识依据，例如图书馆开放、校园卡挂失、宿舍报修等规则。两者结合后，系统既不是只靠模型记忆，也不是简单检索复制，而是在检索资料的约束下生成更自然的回答。
...
选择 LoRA 的原因主要有三点。第一，课程方向要求完成 LoRA/QLoRA 微调，LoRA 能体现对大模型参数高效微调方法的掌握。第二，Qwen2.5-1.5B-Instruct 规模适中，适合在单张 4090D 上完成实验，不需要像全参数微调那样消耗大量显存。第三，校园问答数据量只有 240 条，更适合使用参数高效微调来调整回答风格，而不是对全模型进行大规模训练。
...
实验在 AutoDL 远程服务器完成，工作目录为 `/root/autodl-tmp/llm_workspace`。环境核验结果保存于 `report/evidence/01_env.log`。实际环境为 Python 3.10.20、PyTorch 2.4.0+cu121、Transformers 4.45.2、PEFT 0.19.1、ChromaDB 1.5.9、sentence-transformers 5.6.0。GPU 为 NVIDIA GeForce RTX 4090 D，显存 24564 MiB。项目关键文件包括 `data/campus_qa.json`、`data/campus_docs.json`、`configs/lora_sft.yaml`、`src/rag_utils.py`、`scripts/build_chroma_db.py`、`main.py` 和 `eval_results/compare_12_latest.md`。
...
训练集与验证集划分为 216 条和 24 条。这样的划分保证大部分样本用于训练，同时保留一部分样本观察微调后是否只是在训练集上记忆答案。由于数据规模较小，报告中没有夸大模型泛化能力，而是通过 12 个固定测试问题进一步验证系统在不同类型问题上的表现。

### 4.3 LoRA 微调设计：我具体调整了模型哪里

为了更清楚地展示“如何对模型进行调整”，本项目将微调过程拆分为“冻结基础模型、插入低秩矩阵、训练 adapter、推理时加载 adapter”四个步骤。基础模型 Qwen2.5-1.5B-Instruct 的原始参数保持冻结，不进行全参数更新；训练时只在指定线性层上加入 LoRA 低秩适配矩阵。这样做的意义是：模型原有通用语言能力尽量保留，校园领域知识和回答风格主要通过新增的 adapter 权重注入。
...
| `target_modules` | `all` | 在 LLaMA-Factory 支持的主要线性层中注入 LoRA | 让注意力层和前馈层都能适应校园问答风格 |
...
为避免只描述“使用了 LoRA”而缺少可验证证据，本项目新增 `scripts/inspect_lora_trainable_params.py`，直接从基础模型与 adapter 的 `safetensors` 张量 shape 统计参数量，输出保存在 `report/evidence/12_trainable_params.log`。统计结果显示：基础模型参数量为 `1543714304`（约 `1.543714`B），LoRA adapter 可训练参数量为 `9232384`（约 `9.232384`M），adapter 文件大小为 `35.2679` MB，共包含 `392` 个 LoRA 张量。可训练参数占基础模型比例为 `0.598063%`，占“基础模型 + adapter”总参数比例为 `0.594507%`。
...
参数核验文件包括：`report/evidence/09_lora_config.yaml`、`report/evidence/10_adapter_config.json`、`report/evidence/11_lora_param_explain.md` 和 `report/evidence/12_trainable_params.log`。

这些参数体现了本项目的调参思路：不是盲目把模型训练得越久越好，而是在“小规模校园数据 + 单卡显存 + 保留通用能力”的约束下选择较稳妥的 LoRA 配置。`r=8` 和 `alpha=16` 使 adapter 有足够表达能力学习校园问答结构；`dropout=0.05` 和 `epoch=3` 用于控制过拟合；`batch_size=1` 配合 `gradient_accumulation_steps=8` 解决显存限制；`target_modules=all` 则让模型在更多线性层上获得可训练的领域适配能力。
...
从曲线和日志看，训练过程能够正常记录 loss、learning rate、epoch 和 step，并在训练结束后保存 adapter。需要强调的是，loss 只能证明训练过程正常，不能单独证明模型效果。因此本报告仍以 12 题三模式评估和三组同题对比样例作为主要效果证据。

从训练结果看，本项目完成了一个完整的 SFT 微调闭环：数据集检查通过，训练正常结束，验证集 loss 能够输出，adapter 文件正常保存，后续 LoRA 与 LoRA+RAG 模式均能运行。由于训练集较小，不能只依赖 loss 判断系统好坏，因此本报告进一步采用 12 题测试集进行行为层面的评估。也就是说，loss 证明训练过程有效，三模式回答对比证明微调和检索对实际问答有帮助。

### 4.5 RAG 知识库与 Prompt 组装
...
需要说明的是，课程指导中建议使用 BGE Embedding，但当前远程环境受联网和模型下载条件限制，实际代码使用的是离线 Hash embedding。该方案优点是可复现、无需额外下载、在离线服务器中也能稳定运行；不足是语义泛化能力弱于 BGE 等中文向量模型。为了保证报告真实性，本报告明确写出当前实际方案，不把 Hash 写成 BGE。后续若能补充本地 BGE 模型，应重新构建向量库并使用同一套 12 题评估做对比。
...
评分采用 1 到 5 分制：5 分表示准确、完整、无明显幻觉且符合边界；4 分表示总体正确但有轻微遗漏；3 分表示部分相关但不够完整；2 分表示方向弱相关或有明显缺漏；1 分表示答非所问、幻觉或违反任务边界。评分不是为了证明某个模式一定最好，而是用于观察不同技术模块在不同类型问题上的优势和短板。
...
单独 LoRA 的平均分低于 Base，主要原因是域外问题 Q7 中 LoRA 未能稳定拒答，这说明小规模领域微调可能会增强模型“按校园服务口径回答”的倾向，但不一定提升安全边界能力。因此，高分结论不能写成“LoRA 一定全面优于 Base”，而应写成“LoRA 改善了部分校园问答表达，RAG 明显提升了域内事实准确性，但域外边界仍需额外机制”。这种分析更符合真实实验结果，也比单纯夸大效果更可信。
...
综合来看，本项目已经满足方向 A 对 LoRA 微调和 RAG 知识库的核心要求，并通过三模式对比展示了微调与检索的实际作用。若在最终提交前进一步补充训练曲线截图、LoRA 可训练参数比例、adapter_config 截图和完整演示视频，报告可信度和完成度可以进一步提升。
...
> 最终核验说明：本报告中的 LoRA 参数量、loss 曲线和同题对比均来自远程项目真实文件。当前 RAG 仍为 `HashEmbeddingFunction`，没有改写为 BGE；如后续替换 BGE，必须重建 ChromaDB 并重新运行 12 题评估。
